...
public class DummyAuthorisationEntryDAO extends GenericDaoImpl implements AuthorisationEntriesDAO<DummyAuthorisationEntry> {
	...
	public CreditCardAuthorizationEntry getChild(DummyAuthorisationEntry entry) {
		return getSingleResult(DummyAuthorisationEntry.GET_BY_PARENT_ID, DummyAuthorisationEntry.class, new Param(DummyAuthorisationEntry.parentProp, entry.getId()));
	}

	@Override
	public CreditCardAuthorizationEntry findByAuthorizationCode(String code, Date date) {
		return getSingleResult(DummyAuthorisationEntry.GET_BY_AUTH_CODE, DummyAuthorisationEntry.class, new Param(DummyAuthorisationEntry.authCodeProp, code));
	}

	@Override
	public List<CreditCardAuthorizationEntry> findByDates(Date from, Date to) {
		return getResultList(DummyAuthorisationEntry.GET_BY_DATES, CreditCardAuthorizationEntry.class, new Param(DummyAuthorisationEntry.dateFromProp, from), new Param(DummyAuthorisationEntry.dateToProp, to));
	}

	@Override
	public List<CreditCardAuthorizationEntry> findRefunds(Date from, Date to) {
		return getResultList(DummyAuthorisationEntry.GET_REFUNDS_BY_DATES, CreditCardAuthorizationEntry.class, new Param(DummyAuthorisationEntry.dateFromProp, from), new Param(DummyAuthorisationEntry.dateToProp, to));
	}
	...
	public DummyAuthorisationEntry findById(Integer parentDataPK) {
		return getSingleResultByInlineQuery("from DummyAuthorisationEntry kae where kae.id =:id", DummyAuthorisationEntry.class, new Param("id", parentDataPK));
	}
