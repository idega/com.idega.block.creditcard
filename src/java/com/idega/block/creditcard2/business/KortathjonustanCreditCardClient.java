...
public class KortathjonustanCreditCardClient implements CreditCardClient {
	...
	private String strReferenceNumber = null;// Integer.toString((int)
												// (Math.random() *
												// 43200));
	private KortathjonustanAuthorisationEntry auth = null;
	@Autowired
	private KortathjonustanAuthorisationEntryDAO authDAO = null;

	public KortathjonustanAuthorisationEntryDAO getAuthDAO() {
		if (authDAO == null)
			ELUtil.getInstance().autowire(this);
		return authDAO;
	}

	public void setAuthDAO(KortathjonustanAuthorisationEntryDAO authDAO) {
				...
				if (returnedCaptureProperties != null
					...
					try {
						...
						this.storeAuthorizationEntry(tmpCardNum, null, returnedCaptureProperties,
								KortathjonustanAuthorisationEntry.AUTHORIZATION_TYPE_SALE);
		...
		try {
			...
			try {
				...
				storeAuthorizationEntry(tmpCardNum, parentDataPK, properties,
						KortathjonustanAuthorisationEntry.AUTHORIZATION_TYPE_REFUND);
				log(logText.toString());
	 ...
	 */
	private void storeAuthorizationEntry(String encodedCardnumber, Object parentDataPK, Hashtable properties,
			String authorizationType) throws IDOLookupException, CreateException {
		auth = new KortathjonustanAuthorisationEntry();
		...
		if (properties.containsKey(this.PROPERTY_APPROVAL_CODE))
			auth.setAuthCode(properties.get(this.PROPERTY_APPROVAL_CODE).toString());// authCode);
		...
		if (parentDataPK != null) {
			try {
				auth.setParent((Integer) parentDataPK);
			} catch (Exception e) {
	...
	public String finishTransaction(String properties) throws KortathjonustanAuthorizationException {
		...
		try {
			// String tmpCardNum =
			// CreditCardBusinessBean.encodeCreditCardNumber(cardnumber);
			this.storeAuthorizationEntry(null, null, returnedCaptureProperties,
					KortathjonustanAuthorisationEntry.AUTHORIZATION_TYPE_DELAYED_TRANSACTION);
