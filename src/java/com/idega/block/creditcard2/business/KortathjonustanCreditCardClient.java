...
//	private CreditCardTransaction cct = null;
			...
			if (returnedProperties != null) {
				...
				if (returnedCaptureProperties != null && returnedCaptureProperties.get(this.PROPERTY_APPROVAL_CODE).toString() != null) {
					...
					try {
						...
						this.storeAuthorizationEntry(tmpCardNum, null, returnedCaptureProperties, KortathjonustanAuthorisationEntry.AUTHORIZATION_TYPE_SALE);
	...
	public String doRefund(String cardnumber, String monthExpires, String yearExpires, String ccVerifyNumber, double amount, String currency, Object parentDataPK, String captureProperties) throws CreditCardAuthorizationException {
		...
		try {
			...
			try {
				String tmpCardNum = CreditCardBusiness.encodeCreditCardNumber(cardnumber);
				storeAuthorizationEntry(tmpCardNum, parentDataPK, properties, KortathjonustanAuthorisationEntry.AUTHORIZATION_TYPE_REFUND);
				log(logText.toString());
	...
	private void storeAuthorizationEntry(String encodedCardnumber, Object parentDataPK, Hashtable properties, String authorizationType) throws IDOLookupException, CreateException {
		...
		if (parentDataPK != null) {
			try {
				auth.setParent(getAuthDAO().findById((Integer) parentDataPK));
			}
			catch (Exception e) {
	...
	public String finishTransaction(String properties) throws KortathjonustanAuthorizationException {
		...
		try {
			...
			this.storeAuthorizationEntry(null, null, returnedCaptureProperties, KortathjonustanAuthorisationEntry.AUTHORIZATION_TYPE_DELAYED_TRANSACTION);
