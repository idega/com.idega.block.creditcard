	...
	private void setSecureMerchantData(CreditCardMerchant merchant) {
		...
		try {
			...
			if (!StringUtil.isEmpty(arrOfMerchantProps[0])) {
				LOGGER.info("Found Rapyd merchant login from DOTENV file for the merchant with id: " + merchant.getId() + ". Secure data length: " + arrOfMerchantProps[0].length());
				merchant.setUser(arrOfMerchantProps[0]);
			}
			//PASSWORD
			if (!StringUtil.isEmpty(arrOfMerchantProps[1])) {
				LOGGER.info("Found Rapyd merchant password from DOTENV file for the merchant with id: " + merchant.getId() + ". Secure data length: " + arrOfMerchantProps[1].length());
				merchant.setPassword(arrOfMerchantProps[1]);
			}
			//SHARED SECRET
			if (!StringUtil.isEmpty(arrOfMerchantProps[2])) {
				LOGGER.info("Found Rapyd merchant shared secret from DOTENV file for the merchant with id: " + merchant.getId() + ". Secure data length: " + arrOfMerchantProps[2].length());
				merchant.setSharedSecret(arrOfMerchantProps[2]);
			}
		} catch (Exception e) {
			LOGGER.log(Level.WARNING, "Error setting Rapyd merchant data from DOTENV dir. " + dir + " for merchant: " + merchant, e);
		}
	}
